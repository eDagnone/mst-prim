...
//This should only run for custom test cases, where the Windows line delimiters aren't culled by getline.
std::string killDelimiters(const std::string& s) {
...
int main(){
    ...
    WeightedGraph* graph;
    ifstream infile("test04.in");
    string line;
    while (getline(infile, line)) {
        line = killDelimiters(line);
        if (line == "exit") {
            delete graph;
            return 0;
        }
